let p = document.getElementById("pt");
let k = document.getElementById("key");
let en = document.getElementById("en");
en.addEventListener("click", function () {

    let pt = p.value;
    let key = k.value;

    //key generation
    key = text2bin(key);
    // console.log(key);
    let keyp = [57, 49, 41, 33, 25, 17, 9,
        1, 58, 50, 42, 34, 26, 18,
        10, 2, 59, 51, 43, 35, 27,
        19, 11, 3, 60, 52, 44, 36,
        63, 55, 47, 39, 31, 23, 15,
        7, 62, 54, 46, 38, 30, 22,
        14, 6, 61, 53, 45, 37, 29,
        21, 13, 5, 28, 20, 12, 4];
    key = permute(key, keyp, 56);
    // console.log("hi");
    // console.log(key);
    let shift_table = [1, 1, 2, 2,
        2, 2, 2, 2,
        1, 2, 2, 2,
        2, 2, 2, 1];
    let key_comp = [14, 17, 11, 24, 1, 5,
        3, 28, 15, 6, 21, 10,
        23, 19, 12, 4, 26, 8,
        16, 7, 27, 20, 13, 2,
        41, 52, 31, 37, 47, 55,
        30, 40, 51, 45, 33, 48,
        44, 49, 39, 56, 34, 53,
        46, 42, 50, 36, 29, 32];
    let left = key.substr(0, 28);
    let right = key.substr(28, 28);
    let rkb = []; // rkb for RoundKeys in binary 
    let rk = []; // rk for RoundKeys in text
    for (let i = 0; i < 16; i++) {
        // Shifting 
        left = shift_left(left, shift_table[i]);
        right = shift_left(right, shift_table[i]);

        // Combining 
        let combine = left + right;
        let RoundKey = permute(combine, key_comp, 48);

        rkb.push(RoundKey);

        // rk.push(bin2text(RoundKey));
    }
    encrypt(pt, rkb, rk);
    // console.log(hex2bin(cipher));


    //conversion of text in binary
    function text2bin(key) {
        let bin = "";
        for (let i = 0; i < key.length; i++) {
            let result = key[i].charCodeAt(0).toString(2);
            bin += Array(8 - result.length + 1).join("0") + result;
        }

        return bin;
    }
    //permute
    function permute(k, arr, n) {
        let per = "";
        for (i = 0; i < n; i++) {
            per += k[arr[i] - 1];
        }
        return per;
    }
    //left shift
    function shift_left(k, shifts) {
        let s = "";
        for (i = 0; i < shifts; i++) {
            for (j = 1; j < 28; j++) {
                s += k[j];
            }
            s += k[0];
            k = s;
            s = "";
        }
        return k;
    }
    function hex2bin(s) {
        // hexadecimal to binary conversion 
        let mp = new Map([["0000", "0"], ["0001", "1",], ["0010", "2"], ["0011", "3"], ["0100", "4"], ["0101", "5"], ["0110", "6"], ["0111", "7"], ["1000", "8"], ["1001", "9"], ["1010", "A"], ["1011", "B"], ["1100", "C"], ["1101", "D"], ["1110", "E"], ["1111", "F"]])

        let hex = "";
        for (let i = 0; i < s.length; i += 4) {
            let ch = "";
            ch += s[i];
            ch += s[i + 1];
            ch += s[i + 2];
            ch += s[i + 3];
            hex += mp.get(ch);
        }
        return hex;
    }
    // function bin2text(s) {
    //     let res = parseInt(s, 2).toString(16);
    //     let j = 8;
    //     let i = 0
    //     while (i < 64) {
    //         let sub = s.substring(i, j);
    //         j += 8;
    //         i += 8
    //         let asci = parseInt(sub, 2);
    //         res += String.fromCharCode(asci);

    //     }
    //     return res;
    // }
    //Encryption
    function encrypt(pt, rkb, rk) {
        //text to binary
        pt = text2bin(pt);
        // Initial Permutation Table 
        let initial_perm = [58, 50, 42, 34, 26, 18, 10, 2,
            60, 52, 44, 36, 28, 20, 12, 4,
            62, 54, 46, 38, 30, 22, 14, 6,
            64, 56, 48, 40, 32, 24, 16, 8,
            57, 49, 41, 33, 25, 17, 9, 1,
            59, 51, 43, 35, 27, 19, 11, 3,
            61, 53, 45, 37, 29, 21, 13, 5,
            63, 55, 47, 39, 31, 23, 15, 7];
        // Initial Permutation
        pt = permute(pt, initial_perm, 64);
        let left = pt.substr(0, 32);
        let right = pt.substr(32, 32);
        let exp_d = [32, 1, 2, 3, 4, 5, 4, 5,
            6, 7, 8, 9, 8, 9, 10, 11,
            12, 13, 12, 13, 14, 15, 16, 17,
            16, 17, 18, 19, 20, 21, 20, 21,
            22, 23, 24, 25, 24, 25, 26, 27,
            28, 29, 28, 29, 30, 31, 32, 1];
        let s1 = [
            [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
            [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
            [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
            [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]
        ];
        let s2 = [
            [15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
            [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
            [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
            [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]
        ];
        let s3 = [
            [10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
            [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
            [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
            [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]
        ];
        let s4 = [[7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
        [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
        [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
        [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]
        ];
        let s5 = [[2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
        [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
        [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
        [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]
        ];
        let s6 = [[12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
        [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
        [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
        [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]
        ];
        let s7 = [
            [4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
            [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
            [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
            [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]
        ];
        let s8 = [
            [13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
            [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
            [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
            [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]
        ]
        // Straight Permutation Table
        let per = [16, 7, 20, 21,
            29, 12, 28, 17,
            1, 15, 23, 26,
            5, 18, 31, 10,
            2, 8, 24, 14,
            32, 27, 3, 9,
            19, 13, 30, 6,
            22, 11, 4, 25];
        for (let i = 0; i < 16; i++) {
            // Expansion D-box 
            let right_expanded = permute(right, exp_d, 48);

            // XOR RoundKey[i] and right_expanded 
            let x = xor_(rkb[i], right_expanded);
            // console.log(x);
            let op = "";
            for (let j = 0; j < 8; j++) {
                let row = 2 * parseInt(x[j * 6] - '0') + parseInt(x[(j * 6) + 5] - '0');
                let col = 8 * parseInt(x[(j * 6) + 1] - '0') + 4 * parseInt(x[(j * 6) + 2] - '0') + 2 * parseInt(x[(j * 6) + 3] - '0') + parseInt(x[(j * 6) + 4] - '0');
                let val;
                switch (j + 1) {
                    case 1:
                        val = s1[row][col];
                        break;
                    case 2:
                        val = s2[row][col];
                        break;
                    case 3:
                        val = s3[row][col];
                        break;
                    case 4:
                        val = s4[row][col];
                        break;
                    case 5:
                        val = s5[row][col];
                        break;
                    case 6:
                        val = s6[row][col];
                        break;
                    case 7:
                        val = s7[row][col];
                        break;
                    case 8:
                        val = s8[row][col];
                        break;
                }
                op += Math.floor(val / 8).toString();
                val = Math.floor(val % 8);
                op += Math.floor(val / 4).toString();
                val = Math.floor(val % 4);
                op += Math.floor(val / 2).toString();
                val = Math.floor(val % 2);
                op += Math.floor(val).toString();
            }
            // Straight D-box 
            op = permute(op, per, 32);
            // XOR left and op 
            x = xor_(op, left);
            left = x;
            if (i != 15) {
                [left, right] = [right, left];
            }
        }

        // Combination 
        let combine = left + right;
        // console.log(combine)
        // // Final Permutation Table 
        let final_perm = [40, 8, 48, 16, 56, 24, 64, 32,
            39, 7, 47, 15, 55, 23, 63, 31,
            38, 6, 46, 14, 54, 22, 62, 30,
            37, 5, 45, 13, 53, 21, 61, 29,
            36, 4, 44, 12, 52, 20, 60, 28,
            35, 3, 43, 11, 51, 19, 59, 27,
            34, 2, 42, 10, 50, 18, 58, 26,
            33, 1, 41, 9, 49, 17, 57, 25];
        let cipher = permute(combine, final_perm, 64);
        let mo = document.getElementById("modalbody");
        mo.innerText = hex2bin(cipher);
        $('#cryptModal').modal('toggle');

        console.log(cipher);
    }
    //xor function
    function xor_(a, b) {
        let ans = "";
        for (let i = 0; i < a.length; i++) {
            if (a[i] == b[i]) {
                ans += "0";
            }
            else {
                ans += "1";
            }
        }
        return ans;
    }
})